...
Deno.serve(async (req) => {
  ...
  // Credit the USD value the customer paid. Cryptomus `payment_amount` is in the
  // payer's CRYPTO currency (LTC/BTC/etc.), NOT USD. `payment_amount_usd` is the
  // fiat value. Fall back to `merchant_amount` (already USD for USD invoices) and
  // finally the invoice `amount`.
  const paidAmount = Number(
    body?.payment_amount_usd ?? body?.merchant_amount ?? body?.amount ?? 0,
  );
