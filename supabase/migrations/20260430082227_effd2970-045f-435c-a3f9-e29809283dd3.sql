-- 1. Add sort_index column
ALTER TABLE public.bot_product_stock_items
  ADD COLUMN IF NOT EXISTS sort_index BIGINT;

-- 2. Create sequence and attach as default
CREATE SEQUENCE IF NOT EXISTS public.bot_product_stock_items_sort_index_seq;
ALTER TABLE public.bot_product_stock_items
  ALTER COLUMN sort_index SET DEFAULT nextval('public.bot_product_stock_items_sort_index_seq');

-- 3. Backfill existing rows in current created_at, id order
WITH ordered AS (
  SELECT id, ROW_NUMBER() OVER (ORDER BY created_at, id) AS rn
  FROM public.bot_product_stock_items
...
-- 4. Advance sequence past max
SELECT setval(
  'public.bot_product_stock_items_sort_index_seq',
  COALESCE((SELECT MAX(sort_index) FROM public.bot_product_stock_items), 0) + 1,
  false
);
...
CREATE INDEX IF NOT EXISTS idx_bot_product_stock_items_product_sort
  ON public.bot_product_stock_items (product_id, status, sort_index);

-- 6. Update reserve_internal_stock_items to order by sort_index
CREATE OR REPLACE FUNCTION public.reserve_internal_stock_items(_product_id uuid, _quantity integer, _order_id uuid DEFAULT NULL::uuid)
...
-- 7. Update place_reseller_api_order to order by sort_index
CREATE OR REPLACE FUNCTION public.place_reseller_api_order(_api_key_hash text, _product_id uuid, _quantity integer, _external_order_id text DEFAULT NULL::text)
