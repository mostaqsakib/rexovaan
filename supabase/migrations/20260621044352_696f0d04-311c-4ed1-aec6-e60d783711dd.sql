...
BEGIN
  ...
  SELECT * INTO product_row FROM public.bot_products WHERE id = _product_id AND is_active = true;
  ...
  -- Use caller-supplied unit price if provided (already computed as lowest of
  -- regular / flash / customer special by the edge function), otherwise fall
  -- back to the product's regular price.
  effective_unit := COALESCE(_unit_price, product_row.price);
  IF effective_unit IS NULL OR effective_unit < 0 THEN
    effective_unit := product_row.price;
  END IF;

  computed_total := effective_unit * _quantity;
  ...
  WITH picked AS (
    ...
    WHERE s.product_id = _product_id
      ...
      AND s.data <> '{}'::jsonb
      AND EXISTS (
        SELECT 1 FROM jsonb_each_text(s.data) AS item(key, value)
  ...
  WHERE s.id IN (
    ...
    WHERE picked.product_id = _product_id
      ...
      AND EXISTS (
        SELECT 1 FROM jsonb_each_text(picked.data) AS item(key, value)
        WHERE btrim(coalesce(item.value, '')) <> ''
      )
