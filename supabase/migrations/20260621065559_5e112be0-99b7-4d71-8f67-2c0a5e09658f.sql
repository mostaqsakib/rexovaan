...
BEGIN
  ...
  IF reseller_row.customer_id IS NOT NULL THEN
    ...
    ORDER BY cp.price ASC, cp.min_quantity DESC
    LIMIT 1;
  END IF;

  SELECT fs.sale_price INTO flash_unit
  FROM public.bot_flash_sales fs
  WHERE fs.product_id = product_row.id
  ...
  IF effective_unit IS NULL THEN
