...
BEGIN
  WITH
  paid AS (
    SELECT * FROM bot_orders WHERE status IN ('delivered','completed')
  ),
  rev AS (
    SELECT
  ...
  top_all AS (
    ...
    FROM paid GROUP BY product_name ORDER BY qty DESC LIMIT 5
  ),
  top_week AS (
    SELECT product_name, SUM(quantity)::bigint AS qty, SUM(total_price)::numeric AS revenue
    FROM paid WHERE created_at >= _week GROUP BY product_name ORDER BY qty DESC LIMIT 5
  ),
  top_month AS (
    SELECT product_name, SUM(quantity)::bigint AS qty, SUM(total_price)::numeric AS revenue
    FROM paid WHERE created_at >= _month GROUP BY product_name ORDER BY qty DESC LIMIT 5
  ),
  buyers_all AS (
    ...
    FROM paid o LEFT JOIN bot_customers c ON c.id = o.customer_id
